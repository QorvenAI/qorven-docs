---
title: "Ollama (local)"
description: "Configure Ollama (local) as an LLM backend for your Qors. Setup, models, rate limits, common issues."
...
<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/providers/ollama.svg" alt="Ollama (local) logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Ollama (local)</div>
  </div>
</div>
...
<Steps>
  <Step title="Get credentials from Ollama (local)">
    Install Ollama (local) locally. See their docs.
  </Step>
  <Step title="Add the key in Qorven">
    **Settings → Provider Keys → Add provider → Ollama (local) → paste key → Save.**
