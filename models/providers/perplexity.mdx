...
description: "Configure Perplexity (search) as an LLM backend for your Qors. Setup, models, rate limits, common issues."
sidebarTitle: "Perplexity (search)"
icon: "search"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/providers/perplexity.svg" alt="Perplexity logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Perplexity</div>
  </div>
</div>


<Info>
  Perplexity (search) is available as a provider in Qorven. Add your API key once; rotate across keys automatically on rate-limits.
</Info>
...
<Steps>
  <Step title="Get credentials from Perplexity (search)">
    Create an API key in the Perplexity (search) console.
  </Step>
  <Step title="Add the key in Qorven">
    **Settings → Provider Keys → Add provider → Perplexity (search) → paste key → Save.**
  ...
  <Step title="Mark a default model">
    **Models Hub → Models →** pick which Perplexity (search) model is the default for new Qors.
  </Step>
  <Step title="Test">
    Ask Prime any question. In the LLM trace you'll see `provider=perplexity`.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="401 Invalid key">
    Key is wrong, expired, or from the wrong org. Test with `qorven providers verify perplexity`.
  </Accordion>
  ...
  <Accordion title="Timeout">
    Check your `QORVEN_OUTBOUND_TIMEOUT` (default 120s). Some local models need more. For Perplexity (search) specifically: check your network egress allows the provider's domain.
  </Accordion>
</AccordionGroup>
