---
title: "AWS Bedrock"
description: "Configure AWS Bedrock as an LLM backend for your Qors. Setup, models, rate limits, common issues."
...
<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/providers/bedrock.svg" alt="AWS Bedrock logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">AWS Bedrock</div>
  </div>
</div>


<Info>
  AWS Bedrock is available as a provider in Qorven. Add your API key once; rotate across keys automatically on rate-limits.
</Info>
...
| **Category** | bedrock |
| **API base** | `us-east-1` |
| **Key env var** | `AWS credentials` |
| **Provider type** | Native |

## Setup

<Steps>
  <Step title="Get credentials from AWS Bedrock">
    Create an API key in the AWS Bedrock console.
  </Step>
  <Step title="Add the key in Qorven">
    **Settings → Provider Keys → Add provider → AWS Bedrock → paste key → Save.**
  ...
  <Step title="Mark a default model">
    **Models Hub → Models →** pick which AWS Bedrock model is the default for new Qors.
  </Step>
  <Step title="Test">
    Ask Prime any question. In the LLM trace you'll see `provider=bedrock`.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="401 Invalid key">
    Key is wrong, expired, or from the wrong org. Test with `qorven providers verify bedrock`.
  </Accordion>
  ...
  <Accordion title="Timeout">
    Check your `QORVEN_OUTBOUND_TIMEOUT` (default 120s). Some local models need more. For AWS Bedrock specifically: check your network egress allows the provider's domain.
  </Accordion>
</AccordionGroup>
