---
title: "iMessage"
description: "Connect iMessage to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "iMessage"
icon: "apple"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/imessage.svg" alt="iMessage logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">iMessage</div>
  </div>
</div>


<Info>
  iMessage is a **chat-family** channel. Messages merge into your Qor's canonical chat — the web UI and TUI see them alongside every other chat-family channel.
</Info>
...
|---|---|
| **Family** | chat-family |
| **Setup source** | Blue Bubbles server |
| **Inbound** | Webhook / long-poll / socket — see below |
| **Outbound** | `qorven channels send --channel imessage` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → iMessage**. Paste the credentials from Blue Bubbles server.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/imessage` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from iMessage. Watch it appear in the web UI canonical chat with the iMessage badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound iMessage message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status imessage` — is the binding active?
