---
title: "Web UI"
description: "Connect Web UI to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "Web UI"
icon: "monitor"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/web.svg" alt="Qorven Web UI logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Qorven Web UI</div>
  </div>
</div>


<Info>
  Web UI is a **chat-family** channel. Messages merge into your Qor's canonical chat — the web UI and TUI see them alongside every other chat-family channel.
</Info>
...
| **Outbound** | `qorven channels send --channel web` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → Web UI**. Paste the credentials from built-in.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/web` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from Web UI. Watch it appear in the web UI canonical chat with the Web UI badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound Web UI message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status web` — is the binding active?
