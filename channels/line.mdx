---
title: "LINE"
description: "Connect LINE to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "LINE"
icon: "minus"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/line.svg" alt="LINE logo" width={40} height={40} />
  <div>
...
<Info>
  LINE is a **chat-family** channel. Messages merge into your Qor's canonical chat — the web UI and TUI see them alongside every other chat-family channel.
</Info>
...
|---|---|
| **Family** | chat-family |
| **Setup source** | LINE Developers |
| **Inbound** | Webhook / long-poll / socket — see below |
| **Outbound** | `qorven channels send --channel line` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → LINE**. Paste the credentials from LINE Developers.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/line` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from LINE. Watch it appear in the web UI canonical chat with the LINE badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound LINE message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status line` — is the binding active?
