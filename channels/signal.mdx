---
title: "Signal"
description: "Connect Signal to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "Signal"
icon: "shield"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/signal.svg" alt="Signal logo" width={40} height={40} />
  <div>
...
<Steps>
  ...
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/signal` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from Signal. Watch it appear in the web UI canonical chat with the Signal badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound Signal message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status signal` — is the binding active?
