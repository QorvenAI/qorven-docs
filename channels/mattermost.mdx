---
title: "Mattermost"
description: "Connect Mattermost to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "Mattermost"
icon: "message-square"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/mattermost.svg" alt="Mattermost logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Mattermost</div>
  </div>
</div>


<Info>
  Mattermost is a **chat-family** channel. Messages merge into your Qor's canonical chat — the web UI and TUI see them alongside every other chat-family channel.
</Info>
...
|---|---|
| **Family** | chat-family |
| **Setup source** | Mattermost bot account |
| **Inbound** | Webhook / long-poll / socket — see below |
| **Outbound** | `qorven channels send --channel mattermost` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → Mattermost**. Paste the credentials from Mattermost bot account.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/mattermost` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from Mattermost. Watch it appear in the web UI canonical chat with the Mattermost badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound Mattermost message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status mattermost` — is the binding active?
