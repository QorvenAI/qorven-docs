---
title: "TUI"
description: "Connect TUI to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "TUI"
icon: "terminal"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/tui.svg" alt="Qorven TUI logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Qorven TUI</div>
  </div>
</div>


<Info>
  TUI is a **chat-family** channel. Messages merge into your Qor's canonical chat — the web UI and TUI see them alongside every other chat-family channel.
</Info>
...
| **Outbound** | `qorven channels send --channel tui` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → TUI**. Paste the credentials from built-in.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/tui` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from TUI. Watch it appear in the web UI canonical chat with the TUI badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound TUI message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status tui` — is the binding active?
