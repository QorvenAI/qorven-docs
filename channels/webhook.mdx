...
description: "Connect Generic webhook to your Qor. Setup, webhook URL, message shape, common issues."
sidebarTitle: "Generic webhook"
icon: "webhook"
---

<div className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
  <img src="/images/logos/channels/webhook.svg" alt="Generic webhook logo" width={40} height={40} />
  <div>
    <div className="text-sm font-semibold">Generic webhook</div>
  </div>
</div>


<Info>
  Generic webhook is a **non-chat** channel. Messages live in their own session, not merged into the canonical chat.
</Info>
...
|---|---|
| **Family** | non-chat |
| **Setup source** | any HTTP client |
| **Inbound** | Webhook / long-poll / socket — see below |
| **Outbound** | `qorven channels send --channel webhook` or auto-reply from the agent loop |
...
<Steps>
  <Step title="Create the channel binding in Settings → Channels">
    In the web UI: **Settings → Channels → Add channel → Generic webhook**. Paste the credentials from any HTTP client.
  </Step>
  <Step title="Configure the webhook URL (if the provider pushes events)">
    Qorven exposes `POST /v1/webhooks/webhook` on the web listener. Use your public URL (or a tunnel like Cloudflare / ngrok for dev).
  </Step>
  ...
  <Step title="Test with one message">
    Send a message from Generic webhook. Watch it appear in the web UI canonical chat with the Generic webhook badge.
  </Step>
</Steps>

## Routing & overrides

By default, every inbound Generic webhook message is routed to the Qor bound to this channel. You can override:
...
<AccordionGroup>
  <Accordion title="Messages not arriving">
    - Check `qorven channels status webhook` — is the binding active?
